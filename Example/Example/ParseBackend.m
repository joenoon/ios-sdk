...
- (void)setYGclientKey {
    self.YGclientKey = [[NSUserDefaults standardUserDefaults] objectForKey:@"YGclient_key"];
    
    if (!self.YGclientKey) {
        
        [Parse setApplicationId:ParseApplicationID clientKey:ParseClientKey];
        
        [PFCloud callFunctionInBackground:@"YGgetClientKey" withParameters:nil block:^(NSString *response, NSError *error) {
            if (!error) {
